          ...
          {projeler.map(p => (
            <div key={p.id} onClick={() => setSeciliProjeId(p.id)} style={{ padding: '15px', background: '#f8f9fa', borderRadius: '10px', marginBottom: '10px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #eee' }}>
              ...
              <div style={{display: 'flex', gap: '10px'}}>
                <button onClick={(e) => projeAdiniDuzenle(e, p.id, p.ad)} style={{background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px'}} title="Düzenle">✏️</button>
                <button onClick={(e) => { e.stopPropagation(); projeSil(p.id); }} style={{background: 'none', border: 'none', color: '#e74c3c', cursor: 'pointer', fontSize: '18px'}} title="Sil">🗑️</button>
              </div>
            </div>
          ))}
